...
export default function () {
  ...
  const handleCategorize = async (video: typeof currentlyPlayedVideosSignal.value[0]) => {
    const videoInfo = {
      title: video.title,
          ...
          <Stack gap="xs">
            ...
            <Group gap="xs">
              <Text size="sm" c="dimmed">
                {new Date(video.startTime).toLocaleTimeString()} - {new Date(video.endTime).toLocaleTimeString()}
              </Text>
              
              <Badge 
                variant="light" 
                color={video.categoryType === 'positive' ? 'green' : 
                       video.categoryType === 'negative' ? 'red' : 
                       'gray'}
              >
