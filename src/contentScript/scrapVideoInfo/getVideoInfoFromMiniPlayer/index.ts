...
export default function (): Partial<VideoInfo> {
  const stringWithVideoId =
    document.querySelector('.miniplayer .ytp-tooltip-bg')?.getAttribute('style') ?? undefined

  const videoId = stringWithVideoId ? extractVideoIdFromStyle(stringWithVideoId) : undefined

  const title =
    document.querySelector('.miniplayer #info-bar .title')?.getAttribute('aria-label') ??
    document.querySelector('.miniplayer .title[role="heading"]')?.getAttribute('aria-label') ??
    undefined
  ...
  const channelUrl =
    document.querySelector('.miniplayer .ytp-ce-channel-title')?.getAttribute('href') ?? undefined
